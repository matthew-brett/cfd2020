...
In this next cell, we fetch a routine (actually, a *function*) to generate
random numbers.  We fetch it from the Numpy library, of which you will see
much more.  For now, all you need to know is that the function we want is
called `random_integer` in the Numpy `random` sub-library.  We fetch it like
this:

```{python}
# Fetch the random_integers function from the Numpy library.
from numpy.random import random_integers
```

Now we have the `random_integers` function, we can run the following cell 4 times, to
...
`random_integers` is a *function*.

`random_integers(2)` *calls* the function, and returns a random number, that
is either 0 and 1.

...
We need to reconstruct a new `people` table that has one row for each person
represented in the counts table above.  There are 280 people represented in the
counts table above, of which:
...
We can create this new table in bits, first the "No" / "bystander" rows, then
...
people2['party_yn'] = np.repeat(['No', 'Yes', 'No', 'Yes'], [64, 7, 165, 44])
people2['respondent'] = np.repeat(['bystander', 'rescuer'], [71, 209])
# This is the same as the data frame we made above.
people2.equals(people)
```
...
Luckily, Pandas has a `crosstab` function that does this work for us, for all
four combinations of "Yes", "No" and "bystander", "rescuer".
...
`party_yn` labels, so the pairing of the `party_yn` and `respondent` labels is
random.

```{python}
fake_data = people.copy()
shuffled_party = np.random.permutation(people['party_yn'])
...
`pd.crosstab` will do that for us.

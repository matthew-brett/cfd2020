...
using array calculations.  Don't worry about the details of the function below; as you will soon see, it does the same calculation as the `rmse` function above.
...
Don't worry about the details of the function below - but this function will
quickly give us the same answer as `minimize` above, as long as we are
minimizing the root mean square error for one or more slopes and an intercept.

```{python}
def multiple_regression_algebra(y_values, x_attributes):

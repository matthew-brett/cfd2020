...
Remember, this is a random integer from 0 to 1.

Now let us look at the help for the `np.random.random_integers` function.  As usual, we do this by appending `?` to the function name, and pressing Enter in the notebook.

```{python}
# To see the help for np.random.random_integers, remove the # at the beginning
...
argument called `high` to be 1.

To take another example, in this case we are asking for a random number
starting at 1 up to (including) 10. `low` is 1 and `high` is 10.
...
np.random.random_integers(1, 20, 4)
```
...
np.random.random_integers(low=1, high=20, size=4)
```
...
```{python}
# Four random integers from 1 through 20.
np.random.random_integers(1, 20, size=4)
```

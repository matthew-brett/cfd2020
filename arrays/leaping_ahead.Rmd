...
Here we put together some things you've seen before.  We are creating a Numpy array that has 10000 values, each of which is 0
...
```{python}
# Get an array of 4 integers that can be either 0 or 1
family = np.random.random_integers(0, 1, size=4)
family
```
...
This is a new 10000 element array of True and False values.  It is called a
[Boolean array](../iteration/boolean_arrays).  We will see many more of these.

Next we will use a Numpy function called `count_nonzero`.   It counts the
number of values in an array that are not equivalent to 0.   It turns out that
`False` is equivalent to 0, but `True` is not:

...
```{python}
sns.pairplot(ckd_patients)
```

```{python}
correlation(ckd_patients, 'Serum Creatinine',
            'Blood Urea')
```
...
def rmse_for_params(params):
    ...
    return rmse(intercept, 
                slopes, 
                ckd_patients.loc[:, 'Blood Urea':],
                ckd_patients['Serum Creatinine']
               )
```
...
N = len(ckd_patients)
X = np.ones((N, 3))
X[:, 1:] = ckd_patients.loc[:, 'Blood Urea':]
B = np.linalg.pinv(X) @ ckd_patients['Serum Creatinine']
B
```

```{python}
ax = plt.figure(figsize=(8,8)).add_subplot(111, projection='3d')
ax.scatter(ckdp['Blood Urea'],
           ckdp['Hemoglobin'],
           ckdp['Serum Creatinine'])
ax.set_xlabel('Blood Urea')

...
```{python}
pop_median = np.median(sf2015['Total Compensation'])
pop_median
```
...
```{python}
our_sample = sf2015.sample(n_sample, replace=False)
our_sample['Total Compensation'].plot.hist(bins=sf_bins)
```
...
Recall that when the `sample` method is used without specifying a sample size,
by default the sample size equals the number of rows of the table from which
the sample is drawn.  We could also specify `replace=False` to `sampple`, but
this is the default as well! That's perfect for the bootstrap.  Here is one new
sample drawn from the original sample, and the corresponding sample median.
...
```{python}
resample_1['Total Compensation'].plot.hist(bins=sf_bins)
```
...
By resampling, we have another estimate of the population median. By resampling
again and again, we will get many such estimates, and hence an empirical
...
Let us define a function `bootstrap_median` that takes our original sample, the
label of the column containing the variable, and the number of bootstrap
samples we want to take, and returns an array of the corresponding resampled
medians. 
...
def bootstrap_median(original_sample, label, replications):
    ...
    Parameters
    ----------
    original_sample: data frame
        Data frame containing the original sample
    label: str
        String giving label of column containing the variable.
    replications: number
    ...
    for i in np.arange(replications):
        bootstrap_sample = just_one_column.sample(n, replace=True)
        medians[i] = np.median(bootstrap_sample)
    return medians
```

```{python}
def bs_median(col, replications):
    """Return array of bootstrapped sample medians
    ...
    medians = np.zeros(replications)
    for i in np.arange(replications):
        bootstrap_sample = col.sample(n, replace=True)
        medians[i] = np.median(bootstrap_sample)
    return medians
```

```{python}
timeit bootstrap_median(our_sample, 'Total Compensation', 5000)
```

```{python}
col = our_sample['Total Compensation']
# %timeit bs_median(col, 5000)
```

We now replicate the bootstrap process 5,000 times. The array `bstrap_medians`
contains the medians of all 5,000 bootstrap samples. Notice that the code takes
longer to run than our previous code. It has a lot of resampling to do!
...
The population median of \$110,305 is between these two numbers. The interval
and the population median are shown on the histogram below.
...
We will end up with 100 intervals, and count how many of them contain the
population median.
...
for i in np.arange(100):
    first_sample = sf2015.sample(500, replace=True)
    medians = bootstrap_median(first_sample,
                               'Total Compensation', 
                               5000)
...
plt.plot(make_array(pop_median, pop_median), [0, 100],
         color='red', lw=2)

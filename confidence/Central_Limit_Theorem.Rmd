...
for i in np.arange(n_simulations):
    ...
    winnings_on_red = spin['Winnings: Red']
```
...
```{python}
colors.value_counts().plot.barh()
```

Here are the corresponding winnings:

```{python}
winnings_on_red['Winnings: Red'].plot.hist(bins=np.arange(-1.5, 1.6, 1))
```
...
for i in np.arange(n_simulations):
    ...
    net_gain_red = np.sum(spins['Winnings: Red'].sum())
...
**Center.** The distribution is centered near -\$20, roughly. To see why, note
...
Recall Mendel's probability model for the colors of the flowers of a species of
pea plant. The model says that the flower colors of the plants are like draws
made at random with replacement from {Purple, Purple, Purple, White}.
...
results = pd.DataFrame()
results = results['Sample Proportion: 200'] = props)
results.head()
```

```{python}
results['Sample Proportion: 200'].hist(bins=np.arange(0.65, 0.85, 0.01))
```
...
results = pd.DataFrame()
results = results['Sample Proportion: 800'] = props)
results.head()
```
